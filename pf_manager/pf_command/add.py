...
class AddCommand(BaseCommand):
    ...
    def run(self):
        ...
        if self.params["ssh_server"] is not None:
            ...
            targets[self.params["name"]] = self.__extract_target_from_params()
    ...
    def __extract_target_from_params(self):
        target = {
            "type": self.params["forward_type"], "remote_host": self.params["remote_host"], "name": self.params["name"],
    ...
    def __generate_from_string(self, json_data):
        ...
        if self.params["forward_type"] is None or self.params["forward_type"] == "L":
            json_data[self.params["name"]] = {"type": "L", "remote_host": host, "ssh_server": ssh_server, "name": self.params["name"],
                                    "local_port": first_port, "host_port": second_port, "login_user": login_user}
        elif self.params["forward_type" == "R"]:
            json_data[self.params["name"]] = {"type": self.params["forward_type"], "remote_host": host, "ssh_server": ssh_server,
